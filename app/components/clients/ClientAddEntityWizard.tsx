...
export type ClientAddEntityWizardProps = {
  createdFor: string;
  backLabel: string;
  backHref: string;
...
export default function ClientAddEntityWizard({
  createdFor,
  backLabel,
  backHref,
  onSuccessHref,
  ...
  function updateRow(uid: string, patch: Partial<BeneficiaryRow>) {
    ...
    if (patch.name !== undefined) {
      finalPatch.name = patch.name.replace(/[^a-zA-Z\s]/g, "");
    }
  ...
  function validateForm(): boolean {
    ...
    if (entityType === "trust" && !trustType) {
      setErrorMessage("Please select a trust type.");
      return false;
    }
    if (!entityName.trim()) {
    ...
    if (needsBeneficiaries) {
      if (filledBeneficiaries.length === 0) {
        setErrorMessage(`Please add at least one ${beneficiaryNoun.toLowerCase()}.`);
        return false;
      }
      const hasEmptyName = filledBeneficiaries.some((b) => !b.name.trim());
      const hasInvalidPct = filledBeneficiaries.some((b) => {
        const val = Number.parseFloat(b.percentage);
        return !Number.isFinite(val) || val <= 0;
      });
      if (hasEmptyName) {
        setErrorMessage(`Please enter a name for all ${beneficiaryNounPlural.toLowerCase()}.`);
        return false;
      }
      if (hasInvalidPct) {
        setErrorMessage(`${beneficiaryNoun} ownership percentage must be greater than 0%.`);
        return false;
      }
      if (totalOwnership === 0) {
        setErrorMessage("Total ownership must be greater than 0%.");
        return false;
      }
      if (totalOwnership > 100) {
        setErrorMessage("Total ownership cannot exceed 100%.");
        return false;
      }
    }
    return true;
          ...
          >
            ...
            {entityType === "trust" && (
              <div className="entity-subtype-section" style={{ marginTop: "8px" }}>
                <h3 className="entity-subtype-heading">
                  Select Trust Type:
                </h3>
                <div className="entity-type-grid">
                  {[
            ...
            <div className="entity-beneficiary-list">
              {beneficiaries.map((row) => (
                <div key={row.uid} className="entity-beneficiary-row">
                  ...
                  >
                    <svg viewBox="0 0 24 24" aria-hidden="true" fill="none" stroke="currentColor" strokeWidth="2.0" strokeLinecap="round" strokeLinejoin="round" style={{ width: "16px", height: "16px" }}>
      ...
      <div className="entity-wizard-layout">
        ...
        <div className="entity-wizard-main">
          <div className="entity-wizard-card">
            ...
            {needsBeneficiaries && (
              <div className="entity-wizard-section entity-wizard-section-divider pt-6 mt-2">
                      ...
                      <div className="entity-beneficiary-pct">
                        <input
                          ...
                          onKeyDown={saved ? undefined : (e) => {
                            if (e.key === "-" || e.key === "e" || e.key === "E" || e.key === "+") {
                              e.preventDefault();
                            } else if (e.key === "Enter" && entityName.trim() && entityType && beneficiariesValid && !isSaving) {
                              e.preventDefault();
                              handleSave();
                            }
                          }}
